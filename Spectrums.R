...
# Function; full DHR spectrum with sigma 
my.full.DHR.spectrum <- function(freq, omega.mRNA, 
                                 structures.mRNA, sigma) {
  n.tilda <- length(freq)
  fitted <- rep(0, n.tilda)
  R <- length(omega.mRNA)
  
  fitted <- fitted + sigma[1]/(2*pi)
  for(r in 1:R) {
    fitted <- fitted + sigma[r + 1] * 
      S(freq, omega.mRNA[r], structures.mRNA[r])
  }
  return(fitted)
}
