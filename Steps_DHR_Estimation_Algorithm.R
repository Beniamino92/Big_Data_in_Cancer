...
#### Function: Estimate AR(p) spectrum of observation ####
...
#### Function: Linear Least Square estimate of NVR ####
NVR.Linear.LS <- function(empirical.spectrum, sigma2, 
...
E <- function(NVR, freq, empirical.spectrum, omega.j,
              structures, sigma2) {
...
my.E <- function(NVR, freq, empirical.spectrum, omega.j,
                 structures, sigma2) {
  ...
  X <- matrix(NA, nrow = T, ncol = (R + 1))
  for(r in 1:(R + 1)) {
    X[, r] <- exp(S(freq, omega.j[r], type = structures[r])) 
  }
